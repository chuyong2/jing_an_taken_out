...
### 数据模型
新增菜品，就是将新增页面录入的菜品信息插入到dish表中，如果添加了口味做法，还需要向dish_flavor表（菜品口味表）插入数据
### 代码开发-准备工作
在开发业务功能前，先将需要用到的类和接口基本结构创建好：
 - 实体类DishFlavor（直接从课程资料中导入即可，Dish实体前面课程中已经导入过了）Mapper接口DishFlavorMapper
...
在开发代码之前，需要梳理一下新增菜品时前端页面和服务端的交互过程：
 - 页面（backend/page/food/add.html发送ajax请求，请求服务端获取菜品分类数据并展示到下拉框中
...
### 代码开发
导入dto，用于封装页面提交的数据
那么dto是什么呢，dto全称Data Transfer Object，即数据传输对象，用于展示层和服务层之间的数据传输
